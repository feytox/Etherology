...
/**
 * @see io.wispforest.owo.ui.util.UIErrorToast
 */
public class ErrorToast implements Toast {
    ...
    public ErrorToast(Throwable error) {
    ...
    public static void report(String message) {
        MinecraftClient.getInstance().getToastManager().add(new ErrorToast(message));
    }

    public static void report(Throwable error) {
        MinecraftClient.getInstance().getToastManager().add(new ErrorToast(error));
    }
    ...
    private List<Text> initText(String errorMessage, Consumer<Consumer<Text>> contextAppender) {
        ...
        texts.add(Text.literal(Etherology.MOD_ID + " error").formatted(Formatting.RED));
    ...
    public Object getType() {
        return ErrorToast.Type.ERROR_TOAST;
    }
