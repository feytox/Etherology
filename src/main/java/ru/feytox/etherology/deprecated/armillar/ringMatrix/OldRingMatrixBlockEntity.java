...
public class OldRingMatrixBlockEntity extends BlockEntity implements EGeoBlockEntity {
    ...
    public OldRingMatrixBlockEntity(BlockPos pos, BlockState state) {
        super(EBlocks.RING_MATRIX_BLOCK_ENTITY, pos, state);
    }

    public OldArmillaryMatrixBlockEntity getBaseBlock() {
        if (this.world == null) return null;

        BlockPos armPos = this.pos.add(0, -1, 0);
        return (OldArmillaryMatrixBlockEntity) world.getBlockEntity(armPos);
    }
    ...
    public static void serverTick(World world, BlockPos blockPos, BlockState state, BlockEntity blockEntity) {
        ...
        OldRingMatrixBlockEntity ringMatrix = (OldRingMatrixBlockEntity) blockEntity;
    ...
    public void checkBase(ServerWorld world) {
        ...
        if (!(be instanceof OldArmillaryMatrixBlockEntity)) world.breakBlock(this.pos, false);
    }
