...
public class Chapter {
    ...
    public List<AbstractPage> toPages(TeldecoreScreen screen) {
        ...
        for (AbstractContent content : contents) {
            ...
            if (!page.addContent(content, 6)) {
                Etherology.ELOGGER.error("Found a content in the chapter \"{}\", that doesn't fit anywhere.", title.getString());
                return pages;
            }
            pages.add(page);
        }
    ...
    static {
        TEST = new Chapter(Text.of("Стол Великоестествия"), List.of(
