...
import name.uwu.feytox.lotyh.recipes.visual.TEtherRecipe;
...
public class EtherPage extends EmptyPage {
    public EtherPage(boolean is_left, TEtherRecipe recipe) {
        super(is_left, new LIdentifier("textures/gui/teldecore/teldecore_ether_workbench.png"));
        ...
        for (int i = 0; i < recipe.getShards().size(); i++) {
            ...
            shardGrid.child(
                    new IngredientComponent(new LIngredient(shard))
        ...
        // ingredients
        Map<Integer, LIngredient> matchingItems = recipe.getMatchingItems();
        int finalDx = dx;
