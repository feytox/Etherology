...
public abstract class FeyParticle<T extends FeyParticleEffect<T>> extends SpriteBillboardParticle {
    ...
    @Nullable
    public static <A extends FeyParticle<B> & ParticleInfoProvider<S, A, B>, B extends FeyParticleEffect<B>, S> ParticleInfo<A, B> buildFromInfo(S subtype, A particle, ClientWorld clientWorld, double x, double y, double z, B parameters, SpriteProvider spriteProvider) {
        ParticleInfo.Factory<A, B> infoFactory = particle.getFactory(subtype);
        if (infoFactory == null)
            return null;
