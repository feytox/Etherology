...
public class QuestPage extends TitlePage implements FocusedIngredientProvider {
    ...
    public QuestPage(TeldecoreScreen parent, Quest quest, Identifier chapterId, boolean isLeft) {
        ...
        quest.tasks().forEach(task -> {
            ...
            // TODO: 15.12.2024 TO SLOT
            var slot = ClientTasks.toSlot(task, getPageX() + i * 20 + (isLeft ? 24 : 170), getPageY() + 146 + task.getTaskType().getDy(), 16, 16);
    ...
    private void renderQuestBackground(DrawContext context, float pageX, float pageY) {
        ...
        for (TaskType taskType : TaskType.values())
            renderTaskTypeIcon(context, taskType);

        context.pop();
    }

    private void renderTaskTypeIcon(DrawContext context, TaskType taskType) {
        context.drawTexture(taskType.getTexture(), 0, taskType.getDy(), 0, 0, 118, 12, 118, 12);
    }
